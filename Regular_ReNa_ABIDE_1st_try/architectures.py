...
class FeatureGrouping(nn.Module):
    ...
    def forward(self, x, cluster, training):
        ...
        if training is True:
            reduced = torch.from_numpy(cluster.transform(x.data.numpy())) #x is 2d: Nsbj_in_batch*numb of clusters
            ...
            output = F.linear(reduced_var.float(), weight=self.weight_reduced.float(), bias=self.bias.float()) #sbj in batch * hidden layer
            return output
        else:
